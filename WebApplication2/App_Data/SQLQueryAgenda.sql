...
GO
ALTER TABLE [dbo].[Compras] DROP CONSTRAINT [DF__Compras__DataCom__66603565];
...
GO
PRINT N'Removendo Restrição Padrão restrição sem nome em [dbo].[Orcamentos]...';


GO
ALTER TABLE [dbo].[Orcamentos] DROP CONSTRAINT [DF__tmp_ms_xx__Data___656C112C];
...
GO
ALTER TABLE [dbo].[Agenda] DROP CONSTRAINT [FK__Agenda__Cliente___5535A963];
...
GO
ALTER TABLE [dbo].[Agenda] DROP CONSTRAINT [FK__Agenda__Tatuador__5AEE82B9];
...
GO
/*
O tipo para a coluna Data_Sessao na tabela [dbo].[Agenda] é atualmente  ROWVERSION NOT NULL, mas está sendo alterado para  DATETIMEOFFSET (7) NOT NULL. Não há conversão implícita ou explícita.
*/
GO
PRINT N'Iniciando a recompilação da tabela [dbo].[Agenda]...';
...
CREATE TABLE [dbo].[tmp_ms_xx_Agenda] (
...
IF EXISTS (SELECT TOP 1 1 
           FROM   [dbo].[Agenda])
    BEGIN
        SET IDENTITY_INSERT [dbo].[tmp_ms_xx_Agenda] ON;
        INSERT INTO [dbo].[tmp_ms_xx_Agenda] ([Id], [Cliente_Id], [Tatuador_Id], [Data_Sessao])
        SELECT   [Id],
                 [Cliente_Id],
                 [Tatuador_Id],
                 [Data_Sessao]
        FROM     [dbo].[Agenda]
        ORDER BY [Id] ASC;
        SET IDENTITY_INSERT [dbo].[tmp_ms_xx_Agenda] OFF;
    END

DROP TABLE [dbo].[Agenda];

EXECUTE sp_rename N'[dbo].[tmp_ms_xx_Agenda]', N'Agenda';
...
GO
PRINT N'Criando Restrição Padrão restrição sem nome em [dbo].[Orcamentos]...';


GO
ALTER TABLE [dbo].[Orcamentos]
    ADD DEFAULT SYSDATETIMEOFFSET() FOR [Data_Solicitacao];
...
DECLARE tableconstraintnames CURSOR LOCAL FORWARD_ONLY
    FOR SELECT SCHEMA_NAME([schema_id]),
               ...
               0
        FROM   [sys].[objects]
        WHERE  [parent_object_id] IN (OBJECT_ID(N'dbo.Agenda'))
               AND [type] IN (N'F', N'C')
